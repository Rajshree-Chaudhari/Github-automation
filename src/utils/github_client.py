...
class GitHubClient:
    ...
    def create_pull_request(self, title: str, body: str, head: str, base: str, labels: list = None) -> dict:
        pr = self._request("POST", self._url("/pulls"), json={
            "title": title,
            "body": body,
            "head": head,
            "base": base,
    ...
    def _add_labels(self, pr_number: int, labels: list):
        try:
            self._request("POST", self._url(f"/issues/{pr_number}/labels"), json={"labels": labels})
        except GitHubAPIError as e:
            logger.warning(f"Could not add labels: {e}")

    def add_pr_comment(self, pr_number: int, body: str) -> dict:
        return self._request("POST", self._url(f"/issues/{pr_number}/comments"), json={"body": body})

    def create_review(self, pr_number: int, body: str, event: str, comments: list = None) -> dict:
        """
    ...
    def merge_pull_request(self, pr_number: int, merge_method: str = "squash", commit_title: str = None) -> dict:
